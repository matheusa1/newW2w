...
const MediaInfo = () => {
	...
	const [TvInfo, setTvInfo] = useState<TvInfo | undefined>()
	const [WatchProviders, setWatchProviders] = useState<
		classProviderProps | undefined
	>()
	const [Rate, setRate] = useState<number | undefined>(0)
	const [SimilarMovies, setSimilarMovies] = useState<MovieInfo[] | undefined>(
		[]
	)
	const [SimilarTv, setSimilarTv] = useState<TvInfo[] | undefined>([])
	...
	const resetData = async () => {
		...
		setRate(undefined)
		setSimilarMovies(undefined)
		setSimilarTv(undefined)
	}
	...
	const getMovieInfo = async () => {
		try {
			...
			if (media === 'media') {
				response = await Axios.get(`${moviesURL}${id}?${apiKey}&language=pt-BR`)
	...
	return (
		<S.Container>
			{(media === 'media' && MovieInfo) || (media === 'tv' && TvInfo) ? (
				<>
					<S.LeftSide>
						<S.PosterImage
							src={
